...
async def base_exam_result(
    ...
    if outcome == "pass":
        ...
        if attempts >= 3:
            msg += f"You have been **dismissed** from the LOT Flight Academy due to failing the exam three times in a row. You are allowed to apply for the upcoming hiring waves, thank you & good luck."
        else:
...
async def department_exam_result(
    ...
    if outcome_lower == "pass":
        ...
        if exam_type_lower == "theoretical":
            ...
            msg += (
                f"As you now hold official personnel status, we congratulate you for completing our multi-stage training programme which strives for excellence. Please join the following server(s) as soon as possible.\n"
                f"{personnel_invite if personnel_invite else ''}\n"
                f"{department_invite if department_invite else ''}"
            )
    else:
        msg = (
            f"{EMOJIS['LOTTail_Left']} **{exam_result_title} Results**\n\n"
        ...
        if failed_three_times:
            msg += (
                "You have been **dismissed** from the LOT Flight Academy due to failing the exam three times in a row. You are allowed to apply for the upcoming hiring waves, thank you & good luck."
            )
        else:
            msg += (
